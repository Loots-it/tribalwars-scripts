...
window.FarmGod.Main = (function(Library, Translation) {
        ...
        if (game_data.features.Premium.active && game_data.features.FarmAssistent.active) {
            if (game_data.screen == 'am_farm') {
                $.when(buildOptions()).then((html) => {
                    ...
                    $('.clearOptionReturn').off('click').on('click', function() {
                      $('.optionReturn').val('');
                    });
                    ...
                    $('.optionButton').off('click').on('click', () => {
                        ...
                        let optionReturn = $('.optionReturn').val();
                        let optionMinWall = parseFloat($('.optionMinWall').val());
                        ...
                        localStorage.setItem('farmGod3_options', JSON.stringify({optionGroup: optionGroup, optionDistance: optionDistance, optionTime: optionTime, optionReturn: optionReturn, optionMinWall: optionMinWall, optionWall: optionWall, optionLosses: optionLosses, optionMaxloot: optionMaxloot, optionNewbarbs : optionNewbarbs}));

                        $('.optionsContent').html(UI.Throbber[0].outerHTML + '<br><br>');
                        getData(optionGroup, optionNewbarbs, optionLosses, optionMinWall, optionWall).then((data) => {
                            Dialog.close();

                            let plan = createPlanning(optionDistance, optionTime, optionMaxloot, optionReturn, data);
    ...
    const buildOptions = function() {
        let options = JSON.parse(localStorage.getItem('farmGod3_options')) || {optionGroup: 0, optionDistance: 25, optionTime: 10, optionReturn: '', optionWall: 0, optionLosses: false, optionMaxloot: true, optionNewbarbs: true};
        ...
        return $.when(buildGroupSelect(options.optionGroup)).then((groupSelect) => {
            return `<style>#popup_box_FarmGod{text-align:center;width:600px;}</style>
                ...
                <div style="width:90%;margin:auto;background: url(\'graphic/index/main_bg.jpg\') 100% 0% #E3D5B3;border: 1px solid #7D510F;border-collapse: separate !important;border-spacing: 0px !important;"><table class="vis" style="width:100%;text-align:left;font-size:11px;">
                  ...
                  <tr><td>${t.options.return}</td><td><input type="datetime-local" class="optionReturn" value="${options.optionReturn}"> <input type="button" class="clearOptionReturn" value="X" ></td></tr>
                  <tr><td>${t.options.minWall}</td><td><input type="text" size="5" class="optionMinWall" value="${options.optionMinWall}"></td></tr>
    ...
    const getData = function(group, newbarbs, losses, minWall, wall) {
        ...
        let filterFarms = () => {
            data.farms.farms = Object.fromEntries(Object.entries(data.farms.farms).filter(([key, val]) => {
                return (!val.hasOwnProperty('color')) || (val.wall >= minWall && val.wall <= wall && (val.color != 'red') && (val.color != 'red_blue') && (val.color != 'yellow' || losses));
            }));
    ...
    const createPlanning = function(optionDistance, optionTime, optionMaxloot, optionReturn, data) {
        let plan = {counter : 0, farms : {}};
        let serverTime = Math.round(lib.getCurrentServerTime() / 1000);
        optionReturn = checkReturnTime(optionReturn);
        ...
        for (let prop in data.villages) {
            ...
            orderedFarms.forEach((el) => {
                ...
                let travelTime = (distance * template.speed) * 60;
